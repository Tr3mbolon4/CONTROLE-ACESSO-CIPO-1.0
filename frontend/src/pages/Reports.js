...
import { printDocument, getPrintHeader, getPrintFooter } from '../utils/printUtils';
import { 
  FileText,
  FilePdf,
  FileXls,
...
const Reports = () => {
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ${getPrintHeader(`Relatório de ${getTitle()}`)}
      <div class="summary-box">
        ...
        <div class="summary-item">
          <div class="summary-value">${filters.data_inicio}</div>
          <div class="summary-label">Data Início</div>
        </div>
        <div class="summary-item">
          <div class="summary-value">${filters.data_fim}</div>
          <div class="summary-label">Data Fim</div>
        </div>
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 11px; margin-top: 15px;">
        <thead>
          <tr>
            ${columns.map(col => `<th style="background: #1a1a1a; color: white; padding: 10px 8px; text-align: left; font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px;">${col}</th>`).join('')}
          </tr>
        </thead>
        <tbody>
          ${rows.map((row, idx) => `
            <tr style="background: ${idx % 2 === 0 ? '#fff' : '#f8f9fa'};">
              ${row.map((cell, cidx) => `<td style="border-bottom: 1px solid #e0e0e0; padding: 8px; ${cidx === 0 ? 'font-weight: 600;' : ''}">${cell}</td>`).join('')}
            </tr>
          `).join('')}
        </tbody>
      </table>
